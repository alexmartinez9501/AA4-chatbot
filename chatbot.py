...
def check_all_messages(message):
    ...
    # cursor.execute("SELECT h_respuestas, h_palabrasclave FROM t_historial")
    # t_historial = cursor.fetchall()
    # for h_respuestas,h_palabrasclave in t_historial:
    #     response(h_respuestas, h_palabrasclave, single_response=True)

    # # Respuestas predefinidas{
    # response(respuestas, [palabras_clave], single_response= True, required_words=['saludo'])
    response('Hola,que tal saludos humano soy EduBot Pro', [ 'hi', 'saludos', 'buenas'], single_response=True)
...
root.geometry("400x700+100+50")
root.resizable(width=False, height=False)
root.config(bg="aqua")
history_text = Text(root, wrap="word", width=40, height=25)
history_text.pack()
...
input_label = Label(root, text="Ingresa tu mensaje:",font=("Arial",10,"bold"))
...
logout_button = Button(root, text="Cerrar Sesión",width=16 ,command=logout)
logout_button.pack()
